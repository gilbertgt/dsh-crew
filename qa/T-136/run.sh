#!/usr/bin/env bash
# Runs every QA case of T-136. Exits 0 only when all of them pass.
set -uo pipefail
...
if [ "$cases" -eq 0 ]; then
  echo "FAIL  T-136: no case files found"
  echo "T-136: 0 case(s), 0 passed, 1 failed"
  exit 1
fi

echo "T-136: $cases case(s), $((cases - failed)) passed, $failed failed"
[ "$failed" -eq 0 ]
