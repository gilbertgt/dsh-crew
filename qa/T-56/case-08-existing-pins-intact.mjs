...
//
// THE ONE THAT EARNS THIS CASE. `qa/gaps.md` appears in `roles/pm.md` four
...
// exactly the shape of the `qa/gaps.md` count above. T-65's own report named
...
// values in `docs/design/tasks.md` (pinned by `qa/T-42/case-20`), so a PM
...
const gaps = (text.match(/qa\/gaps\.md/g) ?? []).length;

check(
  "roles/pm.md still points at qa/gaps.md four times",
  gaps >= 4,
...
for (const value of ["code:", "security:", "qa:", "doc:"]) {
  check(
    ...
    "verify-tasks.mjs still demands all four in docs/tasks/ (qa/T-42/case-20), so a PM whose prompt has lost this one meets a red gate with no instructions",
  );
}
...
for (const forbidden of ["{{", "dod.md", "host/git-guard.js", "publishingWorkflow()", "branchPushTriggers()"]) {
  check(
    ...
    forbidden === "{{"
      ...
      : "the PM prompt must not point at this package's own internals — it ships to every project (qa/T-01/case-16)",
  );
}
