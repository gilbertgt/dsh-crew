...
//    or three lines and a line-by-line `grep` misses it. `qa/gaps.md`
...
//    `qa/gaps.md` item 27: an anchor string quoted in a DoD cell is the
...
//    oversight. `qa/gaps.md` item 26 asks whether a sentence that merely
...
for (const { file, before, carriedBy } of SWEPT) {
  ...
  check(
    ...
    + `\n      the per-line count is ${lineHits} and proves nothing on its own (qa/gaps.md item 21)`
