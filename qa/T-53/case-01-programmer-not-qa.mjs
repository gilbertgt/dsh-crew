...
// and QA is a different role whose cases live under `qa/`.
//
...
// let the agent believe it was QA would produce acceptance cases in `qa/`
...
check(
  "the file says QA is a different role, whose cases live under qa/",
  flat(text).includes("QA is a different role") && flat(text).includes("`qa/`"),
  "the file does not point QA's work at qa/",
);

check(
  "it says outright that it writes nothing in qa/ and never writes an acceptance case",
