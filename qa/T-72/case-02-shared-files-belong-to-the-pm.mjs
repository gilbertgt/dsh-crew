...
// files — `qa/run-all.sh` and `qa/gaps.md` — have changed owner. Both
...
// working tree. `qa/gaps.md` and `qa/run-all.sh` are one copy each,
...
//   3. `qa/gaps.md` appears at least once.
//   4. `qa/run-all.sh` appears at least once.
//   5. The shared-file block can still be cut out of the prompt.
//   6. The ownership table's row for `qa/gaps.md` says the file is the PM's
//      and does not say it is QA's.
//   7. The same for the row for `qa/run-all.sh`.
...
//  12. No text just after a mention of `qa/run-all.sh` tells QA to write
...
// still that phrase in a prompt a model reads (`qa/gaps.md` item 30). And
...
// (`qa/gaps.md` item 28). Check 2 exists so this file can never be that
...
const GAPS = "qa/gaps.md";
const RUN_ALL = "qa/run-all.sh";
 ...
 * `qa/run-all.sh` down to the next `## ` heading. Scoped on purpose — the
 * reason has to sit beside the rule it explains, not somewhere else in the file.
 */
function sharedBlock(text) {
  const lines = text.split("\n");
  const start = lines.findIndex((line) => /^\s*\|\s*`qa\/run-all\.sh`/.test(line));
  if (start === -1) return "";
  let end = start + 1;
...
function ownershipClaims(text) {
  const shared = /qa\/(gaps\.md|run-all\.sh)/;
  const found = [];
...
const unlisted = afterBreaking("mutation 4", (dir) => {
  edit(dir, TARGET, "8. the **standing gap list**, `qa/gaps.md`.", "8. the **standing gap list**.");
});
check(
