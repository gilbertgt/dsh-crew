...
// `docs/tasks/` — the one task table, on small work and big work alike,
...
for (const fileName of ["architect.md", "engineer.md", "test-engineer.md", "code-engineer.md", "qa.md", "doc-reviewer.md", "code-reviewer.md", "security-reviewer.md"]) {
  const text = readRoleText(fileName, undefined);
  if (text.includes("dod.md")) fail(`roles/${fileName} names a file called \`dod.md\` (at index ${text.indexOf("dod.md")}) — CRD 0010 forbids that file name anywhere, because a DoD file lives in the job folder and is dropped with it. \`DoD\` is a section of the job's own PRD (docs/design/prd-<date>-<job-slug>.md) or of a task row in docs/tasks/. Point the role at those two places instead`);
  else if (!text.includes("docs/tasks/")) fail(`roles/${fileName} does not name \`docs/tasks/\` — CRD 0010 gives small work and big work one task table in one place, with one shape; only the typist changes (the architect on big work, the PM on small work). Every task row and its DoD section live there, so a role that does not know the path cannot read its own task. Put it back`);
  ...
  else ok(`roles/${fileName} points at docs/tasks/ and knows the DoD section, and names no dod.md`);
}
...

{
  ...
  if (ctx.sections.length !== 1) fail(`expected 1 prompt section, got ${ctx.sections.length}`);
    ...
    else if (copiesOf(pmRulesText, "qa/gaps.md") < 3) fail(`PM section holds only ${copiesOf(pmRulesText, "qa/gaps.md")} copy/copies of \`qa/gaps.md\`, and it needs 3 at least — one of them has been dropped from the PM rules (the core plus every playbook, which is what this pin reads since Crew V2), which carry SEVEN today and give each copy a different job. The jobs are: step 10's review-batching list, where a gap-list entry is named as a document that waits for the last review round instead of blocking a landing; step 11, which STAGES the file so the standing gap list is committed with the task that produced it; step 18's closing migration step, which FILLS it before a single-use document is dropped (the most likely loss: it is one of the places all seven homes of a dropped document are listed — the **Always-loaded invariants** summary is another — and deleting it leaves every other check green); the **Always-loaded invariants** summary, which restates the rule outside the numbered steps; and the routing playbook, which names the path twice. Put the missing copy back`);
  ...
  // path, which is the likeliest way this pin would ever go blind.
  {
    ...
    if (caught.length !== 1 || !perLineBlind) {
      ...
      const places = [
        ["the **Always-loaded invariants** push bullet", slice(flat(ctx.sections[0]?.text ?? ""), "## Always-loaded invariants", "## "), "## Always-loaded invariants"],
