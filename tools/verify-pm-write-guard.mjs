...
try {
  ...
  const whitelistCases = [
    ...
    ["write", "qa/run-all.sh", "the shared QA runner"],
    ["write", "qa/gaps.md", "the standing gap list"],
  ...
  const protectedCases = [
    ...
    ["write", "qa/T-01/case-01-login.mjs", "a QA case"],
    ["write", "qa/T-01/run.sh", "a QA task's own runner (not the shared run-all.sh)"],
  ...
  const childCases = [
    ["write", "src/foo.js", "product code"],
    ["write", "roles/engineer.md", "another role's rules file"],
    ["write", "qa/T-02/case-01.mjs", "a QA case"],
