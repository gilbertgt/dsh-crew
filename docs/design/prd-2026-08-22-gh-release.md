...
这个仓库有 **17 个 tag**（`v0.1.0` 到 `v0.9.0`），**0 个 GitHub release**。

npm 上每个版本都在，`CHANGELOG.md` 里也有写得很细的说明文字，但是 GitHub 的
...
| # | 问题 | 定下来的答案 | 谁决定 |
| --- | --- | --- | --- |
| 1 | 要不要给已有的 17 个 tag 补 release？ | **不补。** 从 `0.10.0` 起自动生成 | 用户 |
...
**代价：** `v0.1.0` 到 `v0.9.0` 在 GitHub 上永远没有 release 页面。要补是一次性的手工
命令，跟这个作业无关。
