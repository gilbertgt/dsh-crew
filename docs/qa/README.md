# docs/qa — QA methodology for a prompt-as-code plugin
...
`bash docs/qa/run-all.sh` between the project checks and the Verdicts gate), and
...
### `docs/qa/run-all.sh` — every task's cases at once

The shared runner does exactly what its header says:

> It finds each docs/qa/*/run.sh by itself, so a new task never needs this file
> edited.
...
`docs/qa/lib` up three), `repoFile(relative)` (one file as text), `pm()` (the PM
