...
// Split on `## ` headings, the shape the file already uses for one gap.
const sections = text.split(/\n(?=## )/).slice(1);
check(`${GAPS}: it is a list of \`## \` sections`, sections.length > 0, `found ${sections.length}`);
...
const matching = sections.filter((body) => NEEDLES.every(({ needle }) => needle.test(body)));
